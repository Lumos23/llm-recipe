...
def main(**kwargs):
    ...
    if "llama" in train_config.model_name.lower():
        ModelDecoderLayer = LlamaDecoderLayer
    elif "gemma" in train_config.model_name.lower():
        ModelDecoderLayer = GemmaDecoderLayer
    elif "mistral" in train_config.model_name.lower():
        ModelDecoderLayer = MistralDecoderLayer
    else: raise NotImplementedError(f"Model {train_config.model_name} not supported")
    

    # Set the seeds for reproducibility
    # torch.cuda.manual_seed(train_config.seed)
    # torch.manual_seed(train_config.seed)
    train_config.seed = random.randint(0, 10000)
    ...
    if train_config.enable_fsdp and train_config.low_cpu_fsdp:
        ...
        if not verify_latest_nightly:
            print("latest pytorch nightly build is required to run with low_cpu_fsdp config, "
    ...
    if train_config.enable_fsdp:
        ...
        mixed_precision_policy, wrapping_policy = get_policies(fsdp_config, rank, model_name=train_config.model_name)
        my_auto_wrapping_policy = fsdp_auto_wrap_policy(model, ModelDecoderLayer)
